...
(
	SELECT DISTINCT 0 AS COI_CONCEPT_ID ,
					CAST('@CONCEPT_SET_NAME' AS VARCHAR(1000)) COI_CONCEPT_NAME ,
					INGREDIENT_CONCEPT_ID CONCEPT_ID,
					INGREDIENT_CONCEPT_NAME CONCEPT_NAME,
					'DRUG' domain_id
	...
	FROM @tableQualifier.DRUG_HOI_EVIDENCE e
		...
		JOIN @tableQualifier.CONCEPT c
			...
			AND c.CONCEPT_ID IN (
				...
				SELECT DISTINCT INGREDIENT_CONCEPT_ID FROM DRUG_UNIVERSE
			)
		JOIN @tableQualifier.LU_CONDITIONS_CHILDREN cc
			...
			AND cc.CONDITION_CONCEPT_ID IN (
				...
				SELECT DISTINCT CONDITION_CONCEPT_ID FROM CONDITION_UNIVERSE
			)
)
SELECT 
	0 CONDITION_CONCEPT_ID
	, CAST('@CONCEPT_SET_NAME' AS TEXT) CONCEPT_SET_NAME
...
GROUP BY @TARGET_DOMAIN_ID_CONCEPT_ID, @TARGET_DOMAIN_ID_CONCEPT_NAME,EVIDENCE_TYPE, MODALITY
...
WITH CTE_SUMMARIZE AS (
	SELECT DISTINCT u.CONCEPT_ID , u.CONCEPT_NAME, u.DOMAIN_ID,
	...
	FROM #COMPLETE_UNIVERSE u
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c1
			ON c1.CONDITION_CONCEPT_ID = u.COI_CONCEPT_ID 
			AND c1.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c1.EVIDENCE_TYPE = 'MEDLINE_MeSH_ClinTrial'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c2
			ON c2.CONDITION_CONCEPT_ID = u.COI_CONCEPT_ID 
			AND c2.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c2.EVIDENCE_TYPE = 'MEDLINE_MeSH_CR'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c3
			ON c3.CONDITION_CONCEPT_ID = u.COI_CONCEPT_ID 
			AND c3.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c3.EVIDENCE_TYPE = 'MEDLINE_MeSH_Other'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c4
			ON c4.CONDITION_CONCEPT_ID = u.COI_CONCEPT_ID 
			AND c4.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c4.EVIDENCE_TYPE = 'MEDLINE_SemMedDB_ClinTrial'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c5
			ON c5.CONDITION_CONCEPT_ID = u.COI_CONCEPT_ID 
			AND c5.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c5.EVIDENCE_TYPE = 'MEDLINE_SemMedDB_CR'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE smo
			ON smo.CONDITION_CONCEPT_ID  = u.COI_CONCEPT_ID 
			AND smo.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND smo.EVIDENCE_TYPE = 'MEDLINE_SemMedDB_Other'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c6
			ON c6.CONDITION_CONCEPT_ID  = u.COI_CONCEPT_ID 
			AND c6.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c6.EVIDENCE_TYPE = 'SPL_EU_SPC'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c7
			ON c7.CONDITION_CONCEPT_ID  = u.COI_CONCEPT_ID 
			AND c7.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c7.EVIDENCE_TYPE = 'SPL_SPLICER_ADR'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c8
			ON c8.CONDITION_CONCEPT_ID  = u.COI_CONCEPT_ID 
			AND c8.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c8.EVIDENCE_TYPE = 'aers_report_count'
		LEFT OUTER JOIN #SUMMARIZE_EVIDENCE c9
			ON c9.CONDITION_CONCEPT_ID  = u.COI_CONCEPT_ID 
			AND c9.DRUG_CONCEPT_ID = u.CONCEPT_ID
			AND c9.EVIDENCE_TYPE = 'aers_report_prr'
	GROUP BY u.CONCEPT_ID , u.CONCEPT_NAME, u.DOMAIN_ID
)
SELECT 
	s.CONCEPT_ID, 
	s.CONCEPT_NAME,
...
FULL OUTER JOIN CTE_SUMMARIZE s ON s.CONCEPT_ID = cu.CONCEPT_ID
ORDER BY s.CONCEPT_ID, s.CONCEPT_NAME, s.DOMAIN_ID;
...
INTO #MODEL_APPLIED
FROM #SUMMARY_SCALED
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b1 ON b1.EVIDENCE_TYPE = 'MEDLINE_MeSH_ClinTrial'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b2 ON b2.EVIDENCE_TYPE = 'MEDLINE_MeSH_CR'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b3 ON b3.EVIDENCE_TYPE = 'MEDLINE_MeSH_Other'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b4 ON b4.EVIDENCE_TYPE = 'MEDLINE_SemMedDB_ClinTrial'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b5 ON b5.EVIDENCE_TYPE = 'MEDLINE_SemMedDB_CR'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b6 ON b6.EVIDENCE_TYPE = 'SPL_EU_SPC'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b7 ON b7.EVIDENCE_TYPE = 'SPL_SPLICER_ADR'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b8 ON b8.EVIDENCE_TYPE = 'aers_report_count'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS b9 ON b9.EVIDENCE_TYPE = 'aers_report_prr'
LEFT OUTER JOIN POSITIVE_NEGATIVE_CONTROL_BETAS i ON i.EVIDENCE_TYPE = 'intercept'
